...
client = MongoClient('mongodb+srv://kumarN:kumarN@kumarn.vxdui.mongodb.net/?retryWrites=true&w=majority&appName=kumarN')  # Replace with your MongoDB URI
db = client['nand']  # Database name
users_collection = db['approved_users']  # Collection name
keep_alive()

# insert your Telegram bot token here
bot = telebot.TeleBot('7896915363:AAFCleyauy3xjqU-Uia7SacqYFDQPF1_c3k')
...
# Function to read user IDs from MongoDB
def read_users():
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        if len(command) > 2:
            ...
            if not users_collection.find_one({"user_id": user_to_add}):
    users_collection.insert_one({"user_id": user_to_add})
...
def remove_user(message):
    ...
    if user_id in admin_id:
        ...
        if len(command) > 1:
            ...
            if users_collection.delete_one({"user_id": user_to_remove}).deleted_count > 0:
...
def show_all_users(message):
    ...
    if user_id in admin_id:
        try:
            user_ids = users_collection.find()
if user_ids:
    ...
    for user in user_ids:
        try:
            ...
            username = user_info.username
            response += f"- @{username} (ID: {user['user_id']})\n"
